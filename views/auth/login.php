...
<style>
  *{box-sizing:border-box;font-family:'Segoe UI',Arial,sans-serif;margin:0;padding:0}
  ...
  button:disabled{background:#94a1b2;cursor:not-allowed;opacity:0.6}
...
<body>
  <div class="login">
    ...
    <?php $bloqueado = ($_SESSION['intentos'] ?? 0) >= 3; ?>
    <form method="POST" action="index.php?accion=procesar-login">
      <label>Usuario</label>
      <input type="text" name="username" <?= $bloqueado ? 'disabled' : 'autofocus' ?> value="<?= htmlspecialchars($_POST['username'] ?? '') ?>">
      <label>Contraseña</label>
      <input type="password" name="password" <?= $bloqueado ? 'disabled' : '' ?>>
